...
export const GoalTimelinePredictor: React.FC<GoalTimelinePredictorProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-[#161817] p-5 sm:p-6 rounded-3xl border border-[#E5E7EB] dark:border-[#242826] shadow-xs space-y-5 text-left transition-colors">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#E5E7EB] dark:border-[#242826] pb-4">
        <div>
          <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#0F6E5F]/10 text-[#0F6E5F] dark:text-[#2DD4BF] text-[11px] font-black uppercase tracking-wider border border-[#0F6E5F]/20 mb-1">
            <Sparkles className="w-3.5 h-3.5" />
            <span>Scientific Goal Timeline Engine</span>
          </div>
          <h3 className="text-lg font-black text-gray-900 dark:text-white flex items-center gap-2">
            <Target className="w-5 h-5 text-[#0F6E5F] dark:text-[#2DD4BF]" />
      ...
      <div className="bg-gradient-to-br from-[#0F6E5F] via-[#0D5B4F] to-[#083D34] rounded-2xl p-5 sm:p-6 text-white relative overflow-hidden shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10 items-center">
          ...
          <div className="grid grid-cols-2 gap-2.5 md:border-l md:border-white/15 md:pl-6">
            <div className="p-3 rounded-xl bg-black/20 backdrop-blur-xs border border-white/10">
              <div className="text-[10px] text-emerald-200 uppercase font-semibold">Total Delta</div>
            ...
            <div className="p-3 rounded-xl bg-black/20 backdrop-blur-xs border border-white/10">
              <div className="text-[10px] text-emerald-200 uppercase font-semibold">Weekly Overload</div>
              <div className="text-base font-black text-white mt-0.5">
            ...
            <div className="p-3 rounded-xl bg-black/20 backdrop-blur-xs border border-white/10">
              <div className="text-[10px] text-emerald-200 uppercase font-semibold">Daily Intake</div>
            ...
            <div className="p-3 rounded-xl bg-black/20 backdrop-blur-xs border border-white/10">
              <div className="text-[10px] text-emerald-200 uppercase font-semibold">Weekly Shift</div>
            ...
            <button
              ...
              className={`p-3 rounded-2xl border text-left transition-all cursor-pointer ${
                pacingMode === 'conservative'
                  ? 'border-[#0F6E5F] dark:border-[#2DD4BF] bg-[#0F6E5F]/5 dark:bg-[#2DD4BF]/10 shadow-xs'
                  : 'border-gray-200 dark:border-[#242826] bg-white dark:bg-[#1A1D1C] hover:border-gray-300'
              }`}
            >
      ...
      <div className="space-y-3">
        <div className="text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-300 flex items-center justify-between">
          <span className="flex items-center gap-1.5">
            <Award className="w-4 h-4 text-[#0F6E5F] dark:text-[#2DD4BF]" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {milestones.map((m, idx) => (
            <div
              key={idx}
              className="p-3.5 rounded-2xl bg-[#FAFAF8] dark:bg-[#1A1D1C] border border-[#E5E7EB] dark:border-[#2A2E2C] flex flex-col justify-between space-y-2 relative"
            >
              <div className="flex items-center justify-between">
                <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-[#0F6E5F]/10 text-[#0F6E5F] dark:text-[#2DD4BF] border border-[#0F6E5F]/20">
                  {m.percentage}% Gate
                </span>
              ...
              <div>
                ...
                <div className="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5 flex items-center justify-between">
                  ...
                  <strong className="text-[#0F6E5F] dark:text-[#2DD4BF]">{m.targetWeight} kg</strong>
                </div>
              </div>
            </div>
          ))}
