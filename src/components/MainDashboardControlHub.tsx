...
export const MainDashboardControlHub: React.FC<MainDashboardControlHubProps> = ({
  ...
  return (
    <section
      ...
      className="mb-6 p-4 sm:p-5 rounded-2xl bg-white dark:bg-[#161817] border border-[#E5E7EB] dark:border-[#242826] shadow-sm transition-all overflow-hidden w-full"
    >
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        {/* Left: User Welcome & Current Physique Target */}
        <div className="flex items-center gap-3.5 min-w-0 w-full xl:w-auto">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#0F6E5F] to-[#0B5247] text-white flex items-center justify-center font-bold text-lg shadow-sm shrink-0">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="font-extrabold text-base sm:text-lg text-[#1A1D1B] dark:text-[#E8ECE9] truncate">
              ...
              <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold ${
                ...
                <span>{isHost ? 'VIP Host Lifetime Free' : activeSub.status === 'active' ? `${activeSub.daysRemaining}d Pro Member` : `${activeSub.daysRemaining}d Pro Trial`}</span>
              </span>
            </div>
            <p className="text-xs text-[#6B7280] dark:text-[#9EA8A2] flex items-center gap-2 flex-wrap mt-0.5">
              <span>Goal: <strong className="text-[#1A1D1B] dark:text-[#E8ECE9] capitalize">{userProfile.goal.replace('_', ' ')}</strong></span>
              <span>•</span>
              <span>Target: <strong className="text-[#0F6E5F] dark:text-[#2DD4BF]">{userProfile.targetWeightKg} kg</strong></span>
              {userProfile.targetDate && (
                <>
                  <span>•</span>
                  <span>Deadline: <strong className="text-[#1A1D1B] dark:text-[#E8ECE9]">{new Date(userProfile.targetDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</strong></span>
                </>
              )}
            </p>
          </div>
        ...
        {/* Right: Quick Action Controls Grid (Fixed 2x2 grid on mobile viewports, flexible inline on tablets/desktops) */}
        <div className="w-full xl:w-auto grid grid-cols-2 sm:flex sm:flex-wrap items-stretch sm:items-center gap-2 sm:gap-2.5">
          ...
          <button
            ...
            className="w-full sm:w-auto min-h-[44px] inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-bold rounded-xl bg-gradient-to-r from-amber-500/15 to-orange-500/15 dark:from-amber-500/20 dark:to-orange-500/20 border border-amber-500/30 dark:border-amber-500/40 text-amber-900 dark:text-amber-300 hover:scale-[1.02] active:scale-95 transition-all shadow-2xs cursor-pointer whitespace-nowrap"
            title="View Training Consistency Streak & 28-Day Consistency Matrix"
          >
            <Flame className="w-4 h-4 text-[#E8912D] fill-[#E8912D] shrink-0" />
          ...
          <button
            ...
            className="w-full sm:w-auto min-h-[44px] inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-bold rounded-xl bg-white dark:bg-[#1E2220] border border-[#E5E7EB] dark:border-[#2A2E2C] text-[#1A1D1B] dark:text-[#E8ECE9] hover:bg-[#F9FAFB] dark:hover:bg-[#282D2A] hover:border-[#0F6E5F] active:scale-95 transition-all shadow-xs cursor-pointer whitespace-nowrap"
            title="Log weekly body weight and trigger automated AI macro plan recalibration"
          >
            <RefreshCw className="w-3.5 h-3.5 text-[#0F6E5F] dark:text-[#2DD4BF] shrink-0" />
          ...
          <button
            ...
            className="w-full sm:w-auto min-h-[44px] inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-bold rounded-xl bg-[#0F6E5F] text-white hover:bg-[#0D5B4F] active:scale-95 transition-all shadow-xs cursor-pointer whitespace-nowrap"
          ...
          <button
            ...
            className="w-full sm:w-auto min-h-[44px] inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-bold rounded-xl bg-white dark:bg-[#1E2220] border border-amber-500/30 text-amber-800 dark:text-amber-300 hover:bg-amber-50 dark:hover:bg-amber-950/30 active:scale-95 transition-all shadow-xs cursor-pointer whitespace-nowrap"
