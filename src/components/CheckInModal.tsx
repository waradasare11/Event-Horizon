...
export const CheckInModal: React.FC<CheckInModalProps> = ({
  ...
  const getRateVerdict = () => {
    ...
    return { text: 'Body Recomposition Steady State', color: 'text-[#00D4FF] bg-[#00D4FF]/10 border-[#00D4FF]/20' };
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white dark:bg-[#0E1424] rounded-3xl max-w-2xl w-full p-5 sm:p-7 shadow-2xl border border-[#E5E7EB] dark:border-[#1E293B] animate-in zoom-in-95 duration-200 text-left my-auto space-y-5 transition-colors">
        
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[#E5E7EB] dark:border-[#1E293B] pb-3.5">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-[#00D4FF]/10 dark:bg-[#00D4FF]/20 text-[#00D4FF] dark:text-[#38BDF8] flex items-center justify-center">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-[#FAFAF8] dark:bg-[#1E2220] border border-[#E5E7EB] dark:border-[#1E293B] text-[#9CA3AF] dark:text-[#9EA8A2] hover:text-[#1A1D1B] dark:hover:text-white flex items-center justify-center transition-all text-xs cursor-pointer"
          >
            ✕
          </button>
        </div>
        ...
        <div className="bg-[#FAFAF8] dark:bg-[#0B0F1E] p-4 sm:p-5 rounded-2xl border border-[#E5E7EB] dark:border-[#1E293B] space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-[#1A1D1B] dark:text-[#E8ECE9] uppercase tracking-wide flex items-center gap-1.5">
                  <Activity className="w-3.5 h-3.5 text-[#00D4FF] dark:text-[#38BDF8]" />
                  Live Progress Trend
                </span>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#00D4FF] text-white animate-pulse">
            ...
            {/* Toggle Metric */}
            <div className="flex items-center gap-1 bg-white dark:bg-[#0E1424] p-1 rounded-lg border border-[#E5E7EB] dark:border-[#1E293B] self-start sm:self-auto">
              <button
                type="button"
          ...
          <div className="grid grid-cols-3 gap-2 pt-1 text-center">
            <div className="p-2 rounded-xl bg-white dark:bg-[#0E1424] border border-[#E5E7EB] dark:border-[#1E293B]">
            ...
            <div className="p-2 rounded-xl bg-white dark:bg-[#0E1424] border border-[#E5E7EB] dark:border-[#1E293B]">
              ...
              <span className="text-sm font-extrabold text-[#00D4FF] dark:text-[#38BDF8]">
                        ...
                        return (
                          <div className="p-2.5 rounded-xl bg-[#1A1D1B] text-white text-xs shadow-lg border border-white/10 space-y-1">
                            <div className="font-bold flex items-center gap-1">
                              <span>{data.date}</span>
                              {data.isLive && (
                                <span className="text-[9px] bg-[#00D4FF] text-white px-1.5 py-0.2 rounded">Live Input</span>
                              )}
                            </div>
                  ...
                  <Area
                    ...
                    dot={(props: any) => {
                      ...
                      if (payload.isLive) {
                        return (
                          <g key={`dot-live`}>
                            <circle cx={cx} cy={cy} r={7} fill="#00D4FF" opacity={0.3} className="animate-ping" />
                            <circle cx={cx} cy={cy} r={5} fill="#00D4FF" stroke="#ffffff" strokeWidth={2} />
                          </g>
                        );
                      }
                      return <circle key={`dot-${cx}-${cy}`} cx={cx} cy={cy} r={3} fill="#00D4FF" />;
                    }}
                  />
                </AreaChart>
              ) : (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="p-3 rounded-2xl bg-white dark:bg-[#0E1424] border border-[#E5E7EB] dark:border-[#1E293B] hover:border-[#00D4FF] dark:hover:border-[#38BDF8] transition-all">
              <label className="block text-xs font-bold text-[#1A1D1B] dark:text-[#E8ECE9] mb-1 flex items-center justify-between">
                <span>Current Weight (kg)</span>
                <span className="text-[10px] text-[#00D4FF] dark:text-[#38BDF8] font-bold">Interactive</span>
              </label>
              <input
                type="number"
                step="0.1"
                ...
                className="w-full text-base font-extrabold text-[#1A1D1B] dark:text-[#E8ECE9] p-2 rounded-xl border border-[#E5E7EB] dark:border-[#1E293B] bg-[#FAFAF8] dark:bg-[#0B0F1E] focus:outline-none focus:ring-2 focus:ring-[#00D4FF]"
                required
              />
                  ...
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded-lg text-xs font-bold border transition-all cursor-pointer ${
                      adherenceScore === score
                        ...
                        : 'bg-[#FAFAF8] dark:bg-[#0B0F1E] border-[#E5E7EB] dark:border-[#1E293B] text-[#6B7280] dark:text-[#9EA8A2]'
                    }`}
                  >
                    {score}★
                  </button>
          ...
          <div className="flex items-center justify-end gap-3 pt-3 border-t border-[#E5E7EB] dark:border-[#1E293B]">
            <button
              type="button"
              onClick={onClose}
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-[#00D4FF] text-white text-xs font-bold hover:bg-[#0369A1] shadow-sm transition-all cursor-pointer"
            >
              {isProcessing ? (
                <>
