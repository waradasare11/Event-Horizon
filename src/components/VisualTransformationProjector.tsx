    ...
    <div className="space-y-8 animate-in fade-in duration-300 text-left">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-2xl border border-[#E5E7EB] shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#6B7280]">Physiological Modeling & Milestones</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-[#E5E7EB] shadow-xs space-y-4">
          <div className="flex justify-between items-center">
            <span className="text-sm font-semibold text-[#374151]">Target Timeline Duration</span>
            <span className="text-sm font-bold text-[#0F6E5F] px-2.5 py-0.5 bg-[#0F6E5F]/10 rounded-full">
              {targetWeeks} Weeks
            </span>
          </div>
          <input
            ...
            className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-[#0F6E5F]"
          />
        ...
        <div className="bg-white p-6 rounded-2xl border border-[#E5E7EB] shadow-xs space-y-4">
          <div className="flex justify-between items-center">
            <span className="text-sm font-semibold text-[#374151]">Assumed Adherence Consistency</span>
            <span className="text-sm font-bold text-amber-700 px-2.5 py-0.5 bg-amber-50 rounded-full">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-[#E5E7EB] shadow-xs space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-lg font-bold text-[#111827]">Physiological Milestones Timeline</h4>
                <p className="text-xs text-[#6B7280] mt-0.5">Click each phase to inspect visual changes, metabolic adaptations, and strength targets</p>
              </div>
              <span className="text-xs font-semibold px-3 py-1 bg-emerald-50 text-[#0F6E5F] rounded-full border border-emerald-200">
                ...
                <button
                  ...
                  className={`p-3.5 rounded-xl border text-left transition-all cursor-pointer ${
                    activeMilestoneTab === idx
                      ...
                      : 'bg-[#F9FAFB] text-[#374151] border-[#E5E7EB] hover:bg-slate-100'
                  }`}
                >
                  <div className={`text-xs font-bold uppercase tracking-wider ${activeMilestoneTab === idx ? 'text-emerald-200' : 'text-[#6B7280]'}`}>
                    Week {m.week}
                  </div>
                  <div className="text-sm font-bold truncate mt-1">
                    {m.predictedWeightKg} kg
                  </div>
                  <div className={`text-xs truncate ${activeMilestoneTab === idx ? 'text-slate-200' : 'text-[#9CA3AF]'}`}>
            ...
            {projectionData.milestones?.[activeMilestoneTab] && (
              <div className="bg-[#F8FAFC] p-6 rounded-xl border border-slate-200 space-y-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-3">
                  <div>
                    <span className="text-xs font-bold uppercase text-[#0F6E5F] tracking-wider">
                      Week {projectionData.milestones[activeMilestoneTab].week} Milestone
                    </span>
                    <h5 className="text-lg font-bold text-[#111827] mt-0.5">
                      {projectionData.milestones[activeMilestoneTab].milestoneName}
                    </h5>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="px-3 py-1 bg-white text-xs font-bold text-slate-700 rounded-lg border border-slate-300">
                      Target: {projectionData.milestones[activeMilestoneTab].predictedWeightKg} kg
                    </span>
                    <span className="px-3 py-1 bg-white text-xs font-bold text-emerald-700 rounded-lg border border-slate-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-1.5">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-[#0F6E5F]">
            ...
            <div className="space-y-3 pt-2">
              <h5 className="text-xs font-bold uppercase text-[#374151] tracking-wider flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {projectionData.adherenceRules?.map((rule: string, rIdx: number) => (
                  <div key={rIdx} className="bg-white p-3.5 rounded-xl border border-[#E5E7EB] text-xs text-[#4B5563] flex items-start gap-2">
                    <CheckCircle className="w-4 h-4 text-[#0F6E5F] shrink-0 mt-0.5" />
                    <span>{rule}</span>
                  </div>
                ))}
              </div>
      ...
      <div className="bg-white p-6 sm:p-8 rounded-2xl border border-[#E5E7EB] shadow-xs space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700">
                Google Maps Grounding
              </span>
              <span className="text-xs text-[#6B7280]">Local Fitness & Nutrition Discovery</span>
            </div>
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-[#F9FAFB] border border-[#E5E7EB] rounded-xl text-sm text-[#111827] focus:outline-hidden focus:ring-2 focus:ring-[#0F6E5F]"
            />
          </div>
          <button
            onClick={handleSearchLocalSpots}
            disabled={isSearchingSpots}
            className="px-5 py-2.5 bg-[#111827] text-white text-xs font-bold rounded-xl hover:bg-black transition-colors flex items-center gap-2 cursor-pointer disabled:opacity-50"
          >
            {isSearchingSpots ? (
        ...
        {localSpotsData && (
          <div className="bg-[#F8FAFC] p-5 rounded-xl border border-slate-200 space-y-4">
            <div className="text-xs text-[#374151] whitespace-pre-line leading-relaxed">
                ...
                <div className="flex flex-wrap gap-2">
                  {localSpotsData.citations.map((c: any, cIdx: number) => (
                    <a
                      ...
                      className="inline-flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 bg-white px-2.5 py-1 rounded-lg border border-slate-200"
                    >
